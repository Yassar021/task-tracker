import { NextRequest, NextResponse } from 'next/server';
import { db } from '@/db';
import { sql } from 'drizzle-orm';

export async function POST() {
  try {
    console.log('=== DEBUG: Cleaning up classes to match specification ===');

    // Define the standard 6 classes per grade
    const standardClasses = [
      // Grade 7
      { id: '7-DIS', grade: 7, name: 'DISCIPLINE' },
      { id: '7-RES', grade: 7, name: 'RESPECT' },
      { id: '7-RESI', grade: 7, name: 'RESILIENT' },
      { id: '7-COL', grade: 7, name: 'COLLABORATIVE' },
      { id: '7-CRE', grade: 7, name: 'CREATIVE' },
      { id: '7-IND', grade: 7, name: 'INDEPENDENT' },

      // Grade 8
      { id: '8-DIS', grade: 8, name: 'DISCIPLINE' },
      { id: '8-RES', grade: 8, name: 'RESPECT' },
      { id: '8-RESI', grade: 8, name: 'RESILIENT' },
      { id: '8-COL', grade: 8, name: 'COLLABORATIVE' },
      { id: '8-CRE', grade: 8, name: 'CREATIVE' },
      { id: '8-IND', grade: 8, name: 'INDEPENDENT' },

      // Grade 9
      { id: '9-DIS', grade: 9, name: 'DISCIPLINE' },
      { id: '9-RES', grade: 9, name: 'RESPECT' },
      { id: '9-RESI', grade: 9, name: 'RESILIENT' },
      { id: '9-COL', grade: 9, name: 'COLLABORATIVE' },
      { id: '9-CRE', grade: 9, name: 'CREATIVE' },
      { id: '9-IND', grade: 9, name: 'INDEPENDENT' },
    ];

    // Get current classes
    const currentClassesResult = await db.execute(sql`
      SELECT id, grade, name FROM classes ORDER BY grade, name
    `);
    const currentClasses = currentClassesResult.rows || [];

    console.log('Current classes in database:', currentClasses);

    // Delete all existing classes
    await db.execute(sql`DELETE FROM classes`);
    console.log('Deleted all existing classes');

    // Insert standard classes
    for (const classData of standardClasses) {
      await db.execute(sql`
        INSERT INTO classes (id, grade, name, is_active, created_at, updated_at)
        VALUES (${classData.id}, ${classData.grade}, ${classData.name}, true, NOW(), NOW())
      `);
    }

    console.log('Inserted 18 standard classes (6 per grade)');

    // Verify the result
    const verifyResult = await db.execute(sql`
      SELECT id, grade, name FROM classes ORDER BY grade, name
    `);

    return NextResponse.json({
      message: 'Classes cleaned up successfully',
      oldClasses: currentClasses,
      newClasses: verifyResult.rows,
      standardClasses: standardClasses,
      debug: {
        totalOld: currentClasses.length,
        totalNew: verifyResult.rows?.length || 0,
        expectedTotal: standardClasses.length
      }
    });

  } catch (error) {
    console.error('CLEANUP CLASSES ERROR:', error);
    return NextResponse.json({
      error: error.message,
      stack: error.stack
    }, { status: 500 });
  }
}